...
.photo-preview {
    ...
    display: {{ $student->photo_path ? 'block' : 'none' }};
    margin-top: 15px;
}
...
<div class="row justify-content-center">
    <div class="col-lg-10">
        ...
        <div class="ethereal-card">
            ...
            <form id="studentEditForm" action="{{ route('students.update', $student->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input name="class_level" class="form-control form-glass" value="{{ $student->class_level }}" placeholder="e.g. V, VI, VII">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input name="institution" class="form-control form-glass" value="{{ $student->institution }}">
                    </div>
                            ...
                            @foreach($student->subject_years ?? [] as $i => $sy)
                                <div class="row mb-3 subject-year-row align-items-end">
                                    <div class="col-md-6">
                                        ...
                                        <select name="subject_years[{{ $i }}][subject_id]" class="form-select form-glass">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input name="subject_years[{{ $i }}][year_label]" value="{{ $sy['year_label'] }}" class="form-control form-glass" placeholder="e.g. PP-1" required>
                                    </div>
                                    <div class="col-md-2">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="small-muted mt-2" style="color: rgba(255,255,255,0.5); font-size: 0.8rem;">
                            Upload or capture via camera. You can crop to portrait format after capturing.
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input id="aadhaarInput" type="file" name="aadhaar" accept="image/*" style="display:none;">
    ...
    <div class="camera-card">
        ...
        <div class="d-flex justify-content-center gap-3 w-100">
            ...
            <button id="cropConfirm" class="btn-confirm-crop">
                <i class="fa-solid fa-check"></i> Crop & Save
            </button>
        </div>
    </div>
</div>
...
document.getElementById('addRow').addEventListener('click', function() {
    ...
    // Note: Using JS generic names here, backend should handle adding new entries
    row.innerHTML = `
        <div class="col-md-6">
            <small class="text-muted d-block mb-1">Subject</small>
            <select name="subject_years[${index}][subject_id]" class="form-select form-glass">
                @foreach($subjects as $s)
        ...
        <div class="col-md-4">
            <small class="text-muted d-block mb-1">Year Label</small>
            <input name="subject_years[${index}][year_label]" class="form-control form-glass" placeholder="e.g. PP-1" required>
        </div>
        <div class="col-md-2">
...
async function startCamera(){
    ...
    try {
        ...
        // --- FIX: Mirror view if using front camera ---
        if (facingMode === 'user') {
            cameraVideo.classList.add('flipped');
        } else {
            cameraVideo.classList.remove('flipped');
        }
...
// 4. Capture & Switch to Crop Modal
captureBtn.addEventListener('click', () => {
    ...
    // Convert to URL and set to cropper image
...
cropConfirm.addEventListener('click', () => {
    ...
    // Get cropped canvas (600x800 for 3:4)
    ...
    // 2. Put into File Input
    canvas.toBlob((blob) => {
        const file = new File([blob], "student-photo.jpg", { type: "image/jpeg", lastModified: new Date().getTime() });
        const container = new DataTransfer();
        container.items.add(file);
        photoInput.files = container.files;

        // Update Label Text
        updateFileName(photoInput, 'photoFileName');
...
// --- UPDATED: Update File Name Text on Selection ---
function updateFileName(input, elementId) {
    const name = input.files[0] ? input.files[0].name : 'Choose File';
    const span = document.getElementById(elementId);
    span.textContent = name;
    span.style.color = input.files[0] ? '#fff' : 'rgba(255,255,255,0.5)';
}

photoInput.addEventListener('change', function(e) {
    updateFileName(this, 'photoFileName');
...
document.getElementById('aadhaarInput').addEventListener('change', function() {
    updateFileName(this, 'aadhaarFileName');
});
</script>
